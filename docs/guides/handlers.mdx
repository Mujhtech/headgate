...
aliases, and collisions before a worker starts polling. Payload versions are decoded when
the job executes; registration does not validate all historical payload shapes.
...
Keep task kinds stable and declare schema versions explicitly in producer envelopes.
Use application-defined upcasters for old payloads and aliases for kind renames.
A codec failure becomes `undecodable`; it does not burn retries that cannot repair bytes.
Go version checks require the optional `Versioned` interface; a plain JSON task does not
enforce them automatically.

<Card title="Task versioning: v1 to v3" icon="code-branch" href="/docs/guides/task-versioning">
  Rename a field, add a required field, implement Go and Rust upcasters, and deploy readers
  before writers. Includes runnable examples and rejection tests.
</Card>
