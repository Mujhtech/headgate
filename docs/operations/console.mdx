...
![Headgate running-job detail sheet showing payload, progress, a resumable checkpoint, and sticky actions](/docs/assets/console-job-detail.jpg)
...
decrypts the payload in the browser. When the embedding application advertises the optional
`payload_reveal` capability, an explicit reveal button asks the server-side authorization
and decryption callback for plaintext. That response is non-cacheable and held only in the
open job view; closing, hiding, or navigating clears it. Reveal is an inspection operation,
so it remains available in a read-only console and never changes the stored job.
...
| --- | --- |
| Overview | Is the fleet healthy, which queue is falling behind, and why is admission blocked? |
