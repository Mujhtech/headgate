...
not presented as a migration backend. SQLite installs its byte-gated embedded schema when
the adapter opens, so it does not use the server-oriented migration CLI.
