...
SQLite uses one database file per installation:
...
Do not point separate logical installations at the same file. The adapter initializes all
durable objects in that file and uses WAL for file-backed databases. File permissions are
the security boundary; `:memory:` is process-local and intended for tests or ephemeral use.

## Security boundary

Separate schemas, databases, prefixes, or SQLite files prevent accidental queue-state collision. They
