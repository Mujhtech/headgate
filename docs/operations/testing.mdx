---
title: "Testing"
description: "Use the in-memory runtime path, embedded SQLite, or isolated live server resources."
icon: "flask-conical"
---
...
| Embedded durable SQL behavior | `headgate_sqlite::SqliteStore::open` with a temporary path | `headgatesqlite.Open` with `t.TempDir()` |
...
- SQLite tests own a file under the test's temporary directory. Opening the adapter installs
  its embedded schema; deleting the temporary directory removes the isolated database.
