...
func TestPlanUpDownAndCurrentNoop(t *testing.T) {
	...
	if err != nil || len(steps) != 13 || steps[0].Migration.Version != 1 || steps[12].Migration.Version != 13 {
		t.Fatalf("up plan = %#v, %v", steps, err)
	}
	current := []AppliedMigration{applied(Postgres, 1), applied(Postgres, 2), applied(Postgres, 3), applied(Postgres, 4), applied(Postgres, 5), applied(Postgres, 6), applied(Postgres, 7), applied(Postgres, 8), applied(Postgres, 9), applied(Postgres, 10), applied(Postgres, 11), applied(Postgres, 12), applied(Postgres, 13)}
	...
	if err != nil || len(steps) != 13 || steps[0].Direction != Down || steps[0].Migration.Version != 13 {
...
func TestChecksumAndHistoryGapFailPlanning(t *testing.T) {
	...
	future := AppliedMigration{Version: 13, Name: "future", Checksum: "x", AppliedAtMS: 1}
...
func TestTargetsAndMaxStepsAreBounded(t *testing.T) {
	fourteen := 14
	if _, err := Plan(Postgres, nil, Up, Options{TargetVersion: &fourteen}); err == nil {
