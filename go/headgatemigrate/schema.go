...
var commonColumns = map[string][]string{
	"headgate_rate_bucket":       {"name", "tokens", "burst", "limit_per_window", "window_ms", "refilled_at_ms"},
	"headgate_quarantine":        {"fingerprint", "kind", "crash_count", "quarantined_at_ms", "sample_payload", "reason"},
	"headgate_partition_deficit": {"queue", "partition_key", "deficit", "updated_at_ms"},
	"headgate_active_partition":  {"queue", "partition_key"},
	"headgate_inflight":          {"queue", "partition_key", "n", "reconciled_at_ms"},
	"headgate_concurrency_limit": {"name", "queue", "max_concurrent", "on_saturated"},
	"headgate_queue_counter":     {"queue", "bucket_ms", "arrived", "completed"},
	"headgate_partition_counter": {"queue", "partition_key", "bucket_ms", "arrived", "completed"},
	"headgate_queue_state":       {"queue", "paused", "weight", "dispatch_count"},
...
var postgresJobColumns = []string{
	...
	"unique_key", "unique_states", "unique_expires_at_ms", "headers", "errors",
	"checkpoint", "cp_cursor", "result_schema_version", "result_bytes",
	"output_schema_version", "output_bytes", "output_fence", "output_updated_at_ms",
	"progress_current", "progress_total", "progress_message", "progress_fence", "progress_updated_at_ms",
	"periodic_schedule_id", "periodic_tick_ms", "sticky_worker",
}

var mysqlJobColumns = []string{
	"id", "ulid", "kind", "schema_version", "payload", "queue", "state",
	"partition_key", "rate_class", "weight", "rate_charge", "fingerprint",
	"priority", "attempt", "crash_attempt", "max_attempts", "enqueued_at_ms",
	"scheduled_at_ms", "timeout_ms", "deadline_ms", "retention_ms", "finalized_at_ms",
	...
	"output_fence", "output_updated_at_ms",
	"progress_current", "progress_total", "progress_message", "progress_fence", "progress_updated_at_ms",
	"periodic_schedule_id", "periodic_tick_ms", "sticky_worker",
}

var postgresIndexes = []string{
	"headgate_job_admit",
	"headgate_job_lease",
