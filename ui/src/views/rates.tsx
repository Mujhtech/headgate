...
import { Empty, Failure, Loading, useApiResource, type ViewProps } from "@/console"
...
export function RatesView({ refreshKey, refresh, notify }: ViewProps) {
  const { data, error, loading } = useApiResource<{ rate_classes?: RateClass[] } | RateClass[]>("/rate-classes", refreshKey)
  ...
  return <>
    ...
    <RateDialog value={editing} open={editing !== undefined} close={() => setEditing(undefined)} refresh={refresh} notify={notify} />
  </>
}

function RateDialog({ value, open, close, refresh, notify }: { value: RateClass | null | undefined; open: boolean; close: () => void; refresh: () => void; notify: ViewProps["notify"] }) {
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    const fields = new FormData(event.currentTarget)
    const name = String(fields.get("name") ?? "").trim()
    try {
      await api(`/rate-classes/${encodeURIComponent(name)}`, { method: "PUT", body: { limit: Number(fields.get("limit")), window_ms: Number(fields.get("window")), burst: Number(fields.get("burst")), paused: fields.get("paused") === "on" } })
      notify("Rate class saved"); close(); refresh()
  ...
  return <Dialog open={open} onOpenChange={(next) => !next && close()}><DialogContent className="inset-auto left-1/2 top-1/2 h-auto max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border"><DialogHeader><DialogTitle>{value ? `Edit ${value.name}` : "New rate class"}</DialogTitle><DialogDescription>Configure the shared budget and its operational kill switch.</DialogDescription></DialogHeader><form onSubmit={submit} className="grid gap-4">
    ...
    <label className="flex items-center gap-2 text-sm"><input name="paused" type="checkbox" defaultChecked={value?.paused} />Paused</label>
