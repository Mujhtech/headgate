...
export function SchedulesView({ notify }: ViewProps) {
  ...
  const run = async (schedule: Schedule) => { try { await scheduleMutation.mutateAsync({ path: `/periodic/${encodeURIComponent(schedule.id)}/run` }); notify("Schedule fired") } catch (reason) { notify(reason instanceof Error ? reason.message : String(reason), "error") } }
  ...
  return <><div className="mb-4"><h1 className="text-lg font-semibold">Periodic schedules</h1><p className="text-sm text-muted-foreground">Durable, store-coordinated recurring work and its enqueue history.</p></div><Card><CardHeader><CardTitle>Schedules</CardTitle></CardHeader><CardContent>{schedules.length ? <Table><TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Spec</TableHead><TableHead>Kind → queue</TableHead><TableHead>Next run</TableHead><TableHead>Missed</TableHead><TableHead><span className="sr-only">Actions</span></TableHead></TableRow></TableHeader><TableBody>{schedules.map((schedule) => { const basePath = `/periodic/${encodeURIComponent(schedule.id)}`; return <TableRow key={schedule.id}><TableCell className="font-mono text-xs">{schedule.id} {schedule.paused && <Badge variant="destructive">paused</Badge>}</TableCell><TableCell className="font-mono text-xs">{schedule.spec}</TableCell><TableCell>{schedule.kind} → {schedule.queue}</TableCell><TableCell>{formatDate(schedule.next_run_ms)}</TableCell><TableCell>{schedule.on_missed}</TableCell><TableCell><div className="flex flex-wrap gap-1"><Button variant="outline" size="sm" onClick={() => setSelected(schedule.id)}>Events</Button><ActionButton variant="outline" size="sm" disabled={config.readOnly || scheduleMutation.isPending} pending={scheduleMutation.isPending && scheduleMutation.variables?.path === `${basePath}/run`} pendingLabel="Running…" onClick={() => void run(schedule)}>Run now</ActionButton><ActionButton variant="destructive" size="sm" disabled={config.readOnly || scheduleMutation.isPending} pending={scheduleMutation.isPending && scheduleMutation.variables?.path === basePath} pendingLabel="Deleting…" onClick={() => void remove(schedule)}>Delete</ActionButton></div></TableCell></TableRow> })}</TableBody></Table> : <Empty>No schedules configured.</Empty>}</CardContent></Card></>
}
