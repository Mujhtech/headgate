...
export function WorkersView({ refreshKey, refresh, notify }: ViewProps) {
  const workersResource = useApiResource<{ workers?: Worker[] } | Worker[]>("/workers", refreshKey)
  const clusterResource = useApiResource<Cluster>("/cluster", refreshKey)
  ...
  const signal = async (worker: Worker, command: string) => {
    ...
    try { await mutate(`/workers/${encodeURIComponent(worker.worker_id)}/signal`, { body: { command } }); notify(`Worker command sent: ${command}`); refresh() } catch (reason) { notify(reason instanceof Error ? reason.message : String(reason), "error") }
  }
  return <>
