...
import { workerActionDisabledReason, type WorkerCommand, type WorkerStatus } from "@/lib/worker-control"

interface Worker { worker_id: string; host?: string; queues?: string[]; inflight?: number; concurrency: number; heartbeat_at_ms: number; status?: WorkerStatus; duties_active?: boolean; pending_command?: WorkerCommand | null }
...
export function WorkersView({ notify }: ViewProps) {
  ...
  return <>
    ...
    <Card><CardHeader><CardTitle>Workers</CardTitle></CardHeader><CardContent>{workers.length ? <Table><TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Host</TableHead><TableHead>Queues</TableHead><TableHead>Status</TableHead><TableHead>Slots</TableHead><TableHead>Heartbeat</TableHead><TableHead><span className="sr-only">Actions</span></TableHead></TableRow></TableHeader><TableBody>{workers.map((worker) => <TableRow key={worker.worker_id}><TableCell className="font-mono text-xs">{worker.worker_id}</TableCell><TableCell>{worker.host ?? "—"}</TableCell><TableCell><div className="flex flex-wrap gap-1">{(worker.queues ?? []).map((queue) => <Badge key={queue} variant="outline">{queue}</Badge>)}</div></TableCell><TableCell><div className="flex flex-wrap gap-1"><Badge variant="outline">{worker.status ?? "running"}</Badge>{worker.pending_command ? <Badge variant="secondary">pending: {worker.pending_command}</Badge> : null}{worker.duties_active === false ? <Badge variant="secondary">duties resigned</Badge> : null}</div></TableCell><TableCell>{worker.inflight ?? 0}/{worker.concurrency}</TableCell><TableCell>{formatDate(worker.heartbeat_at_ms)}</TableCell><TableCell><div className="flex flex-wrap gap-1">{(["quiet", "resume", "restart", "resign", "terminate"] as const).map((command) => { const reason = workerActionDisabledReason(worker, command); return <span key={command} title={reason ?? undefined}><ActionButton variant={command === "terminate" ? "destructive" : "outline"} size="sm" disabled={config.readOnly || signalMutation.isPending || Boolean(reason)} pending={pendingCommand === `${worker.worker_id}:${command}`} pendingLabel="Sending…" onClick={() => void signal(worker, command)}>{command === "restart" ? "Rolling restart" : command === "resign" ? "Resign duties" : command[0].toUpperCase() + command.slice(1)}</ActionButton></span> })}</div></TableCell></TableRow>)}</TableBody></Table> : <Empty>No live workers.</Empty>}</CardContent></Card>
  </>
}
